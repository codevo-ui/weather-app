...
import { MapPin, Eye, Wind, Droplets, Gauge, Thermometer, Sunset, Sunrise } from 'lucide-react';
import { getWeatherIcon, formatTemperature, formatTime } from "../utils/weatherutils";
import * as LucideIcons from "lucide-react";

const WeatherCard = ({ weather, unit }) => {
  // ✅ Guard check to prevent crash if weather is null
  if (!weather || !weather.weather) {
    return (
      <div className="text-white p-6 bg-white/10 rounded-3xl text-center">
    ...
    <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-8 shadow-2xl hover:bg-white/15 transition-all duration-500">
      ...
      <div className="flex items-center justify-between mb-10">
        <div className="flex-1">
          <div className="text-7xl font-bold text-white mb-3 tracking-light">
        ...
        {WeatherStats.map((stat, index) => (
          ...
          >
            <div className="flex items-center space-x-3 mb-2">
              ...
              <span className="text-white/70 text-sm font-medium">{stat.lable}</span>
            </div>
